...
const SEVERITY_STYLES: Record<string, string> = {
  high:   "border-red-500/30    bg-red-500/8    dark:bg-red-500/8",
  medium: "border-yellow-500/30 bg-yellow-500/6 dark:bg-yellow-500/6",
  low:    "border-green-500/30  bg-green-500/6  dark:bg-green-500/6",
};
...
function Modifier({ label }: { label: string }) {
  return (
    <span className="rounded px-1.5 py-0.5 text-[10px] font-mono font-medium bg-zinc-100 dark:bg-zinc-700/60 text-zinc-600 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-600/40">
      {label}
    </span>
  );
}
...
function InnerCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-0.5 rounded-lg bg-zinc-50 dark:bg-zinc-800 px-3 py-2 border border-zinc-200 dark:border-zinc-700/40">
      {children}
    </div>
  );
}
    ...
    <div className="grid grid-cols-1 gap-4 sm:gap-5 lg:grid-cols-2">
      ...
      <div className="lg:col-span-2">
        <Section title="Contract Summary" icon="📋">
          <p className="text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">{report.summary}</p>
        </Section>
      </div>

      {/* Risk Areas — full width */}
      <div className="lg:col-span-2">
              ...
              <div key={i} className={`rounded-lg border px-4 py-3 flex flex-col gap-2 ${SEVERITY_STYLES[r.severity] ?? SEVERITY_STYLES.low}`}>
                ...
                <div className="flex flex-col gap-1 pl-1">
                  <p className="text-xs text-zinc-700 dark:text-zinc-300 leading-relaxed">
                    <span className="font-semibold text-zinc-500 dark:text-zinc-400 uppercase text-[10px] tracking-wide mr-1">Attack path</span>
                    {r.attackPath}
                  </p>
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                    <span className="font-semibold text-zinc-400 dark:text-zinc-500 uppercase text-[10px] tracking-wide mr-1">Fix</span>
                    {r.recommendation}
                  </p>
                </div>
              </div>
          ...
          {report.attackSurface.map((fn, i) => (
            <InnerCard key={i}>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="rounded px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide bg-violet-100 dark:bg-violet-500/10 text-violet-700 dark:text-violet-400 border border-violet-200 dark:border-violet-500/20">
                  {fn.visibility}
                </span>
                {fn.modifiers?.map((m, j) => <Modifier key={j} label={m} />)}
              </div>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 pt-0.5">{fn.description}</p>
            </InnerCard>
          ))}
        </div>
      </Section>
      ...
      <Section title="Modifier Map" icon="🗺️">
        <div className="flex flex-col gap-2">
          {report.modifierMap?.map((m, i) => (
            <InnerCard key={i}>
              <div className="flex items-center gap-2 flex-wrap">
                <code className="text-xs font-bold text-cyan-600 dark:text-cyan-400">{m.function}()</code>
                {m.modifiers.length > 0
                  ? m.modifiers.map((mod, j) => <Modifier key={j} label={mod} />)
                  : <span className="text-[10px] text-zinc-400 dark:text-zinc-600 italic">no modifiers</span>
                }
              </div>
              {m.note && (
                <p className="text-[11px] text-zinc-400 dark:text-zinc-500 pt-0.5">{m.note}</p>
              )}
            </InnerCard>
          ))}
        </div>
          ...
          {report.keyVariables.map((v, i) => (
            <InnerCard key={i}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-zinc-400 dark:text-zinc-500 font-mono">{v.type}</span>
              </div>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 pt-0.5">{v.risk}</p>
            </InnerCard>
          ))}
        </div>
      </Section>
              ...
              <div
                ...
                className={`rounded-lg px-3 py-2 border ${
                  v.present
                    ...
                    : "bg-zinc-50 dark:bg-zinc-800/60 border-zinc-200 dark:border-zinc-700/40"
                }`}
              >
                <div className="flex items-center gap-2 mb-0.5">
                  <span className={`text-sm ${v.present ? "text-red-500 dark:text-red-400" : "text-zinc-400 dark:text-zinc-500"}`}>
                    {v.present ? "✗" : "✓"}
                  </span>
                  <span className={`text-xs font-semibold ${v.present ? "text-red-700 dark:text-red-300" : "text-zinc-500 dark:text-zinc-400"}`}>
                    {v.pattern}
                  </span>
                </div>
                <p className="pl-5 text-[11px] text-zinc-400 dark:text-zinc-500">{v.detail}</p>
              </div>
            ))}
          </div>
        </Section>
